...
```console
$ docker run --name some-nginx -v /some/content:/usr/share/nginx/html:ro -d rodinvr/nginx
```
...
```console
$ docker run --name my-custom-nginx-container -v /host/path/nginx.conf:/etc/nginx/nginx.conf:ro -d rodinvr/nginx
```
...
```console
$ docker run --name tmp-nginx-container -d rodinvr/nginx
...
```console
$ docker run --name my-nginx -v /host/path/nginx.conf:/etc/nginx/nginx.conf:ro -d rodinvr/nginx nginx-debug -g 'daemon off;'
```
...
The `rodinvr/nginx` images come in many flavors, each designed for a specific use case.

## `rodinvr/nginx:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `rodinvr/nginx:alpine`
